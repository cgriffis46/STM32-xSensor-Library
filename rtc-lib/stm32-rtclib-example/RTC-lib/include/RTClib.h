...
#define PCF8523_ADDR 0x00
#define PCF8563_ADDR 0x00
...
public:
    RTC_DS1307(I2C_HandleTypeDef* _hi2c);
...
public:
    RTC_DS3231(I2C_HandleTypeDef* _hi2c);
  bool begin(uint8_t devAddress);
  void adjust(const DateTime &dt);
  bool lostPower(void);
  DateTime now();
...
public:
  RTC_PCF8523(I2C_HandleTypeDef* _hi2c);
...
public:
  RTC_PCF8563(I2C_HandleTypeDef* _hi2c);
  bool begin(uint8_t  WriteAdress, uint8_t ReadAddress);
  bool lostPower(void);
  void adjust(const DateTime &dt);
  DateTime now();
